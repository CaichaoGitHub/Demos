//
//  EvenEvenMoreLighting.cpp
//  StudyOpenGL
//
//  Created by caichao on 16/10/15.
//  Copyright © 2016年 Caichao. All rights reserved.
//

#include "EvenMoreLighting.hpp"

#include "platform.hpp"

double EvenMoreLighting::gScrollY = 0; //static variable should be initilized in source file

EvenMoreLighting::EvenMoreLighting()
{
    window = nullptr;
    gScrollY = 0;
    degreesRotated = 0;
...
tdogl::Program* EvenMoreLighting::loadShaders()
{
    std::vector<tdogl::Shader> shaders;
    shaders.push_back(tdogl::Shader::shaderFromFile(ResourcePath("more_lighting_vertex_shader.glsl"), GL_VERTEX_SHADER));
    shaders.push_back(tdogl::Shader::shaderFromFile(ResourcePath("more_lighting_fragment_shader.glsl"), GL_FRAGMENT_SHADER));
    return new tdogl::Program(shaders);
}

tdogl::Texture* EvenMoreLighting::loadTexture()
{
...
void EvenMoreLighting::loadWoodenCrateAsset()
{
    ...
    woodenCreate.shininess = 80.0;
    woodenCreate.specularColor = glm::vec3(1.0,1.0,1.0);
...
void EvenMoreLighting::createInstance()
{
    ModelInstance dot;
...
void EvenMoreLighting::renderInstance(const ModelInstance &inst)
{
...
void EvenMoreLighting::update(float secondsElapsed)
{
...
void EvenMoreLighting::onScroll(GLFWwindow *window, double deltax, double deltay)
{
    gScrollY += deltay;
}

void EvenMoreLighting::onError(int errorCode, const char *msg)
{
    throw std::runtime_error(msg);
}

void EvenMoreLighting::appMain()
{
